            ...
            page.getTextContent().then((textContent) => {
                ...
                for (let i = 0; i < textItems.length; i++) {
                    ...
                    if (textItems[i].str.includes("eSocial - Evento S-2500")) {
                        ...
                        let rightCellValue = findRightCellValue(textItems, i); // Atualiza a variável personalizada
                        if (rightCellValue !== null) {
                            // Verifica se o valor é 0 e substitui por "ISENTO"
                            let displayValue = (rightCellValue === 0) ? "ISENTO" : formatBrazilianCurrency(rightCellValue);
                            addRowToTable(getNewLabel("IRPF DEVIDO PELO RECLAMANTE", occurrenceCount["IRPF DEVIDO PELO RECLAMANTE"]), displayValue);
                        } else {
...
function addTextAboveTable() {
    
    let container = document.getElementById('resultsContainer');
    
    let text = `id. XXXXX, para fixar o valor da execução no total de R$ XXXXXX, corrigido monetariamente e com incidência de juros legais até XXXXX, nos valores abaixo discriminados: , responsabilizando-se a(s) outra(s) ré(s) subsidiariamente / solidariamente:`;
...
// Adiciona o texto acima da tabela  
addTextAboveTable();